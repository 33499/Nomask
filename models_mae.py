...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        x_delete = torch.cat((cls_tokens, x_delete), dim=1)
        ...
        for blk in self.blocks:
            x_delete = blk(x_delete)
        x_delete = self.norm(x_delete)
        # delete cls token
        x_visible = x_visible[:,1:,:]
        x_delete = x_delete[:,1:,:]

        # x_visible = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))
        # x_delete = torch.gather(x, dim=1, index=ids_delete.unsqueeze(-1).repeat(1, 1, D))
        ...
        return x_visible,x_delete, mask, ids_restore, ids_keep, ids_delete, x_delete_pos_embeds, x_visible_pos_embeds
